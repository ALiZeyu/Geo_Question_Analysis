...
public class YingXiangTemplate2 {
	...
	public static String getTemplate(TimuInfo topic, List<String> senList) throws IOException {
		...
		Map<String, String> cueword = Input.getCueword(12);
		List<Tree> word = findCueWord.getCueWords(cueword, t, 12, bound, sentence);
		String template = "";
		Set<String> words = new HashSet<String>();
		for (String s : sentence)
			words.add(s.split("_")[0]);
		if(topic.tgTag.contains("措施_NN") && topic.tgTag.contains("是_VC"))
			return "";
		for (Tree w : word) {
			...
			else if ((words.contains("受") || words.contains("受到")) && (MyUtil.isAtEnd(w.no, sentence)||sentence.get(w.no).endsWith("_VA")) && MyUtil.strEquals(w.content, "控制|限制|侵扰"))
				template += w.no + "+" + modifySlot(YingXiangSolution1.print(w, sentence, topic.sxwText), words) + "\t";
			...
			//对。。重要，重要要在句尾
			else if (words.contains("对") && w.content.equals("重要") && w.no==sentence.size())
				template += w.no + "+" + modifySlot(YingXiangSolution4.print(w, sentence), words) + "\t";
			else if ((w.content.equals("有关") || w.content.equals("相关") || w.content.equals("无关")) && words.contains("与"))
				template += w.no + "+" + modifySlot(YingXiangSolution5.print(w, sentence), words) + "\t";
